...
public interface UserRepository extends JpaRepository<UserEntity, Long> {
    ...
    @Query("SELECT u FROM UserEntity u WHERE (:name IS NULL OR u.username LIKE %:name%) AND (:pin IS NULL OR u.address.pinCode = :pin) AND (:start IS NULL OR u.registrationDate >= :start) AND (:end IS NULL OR u.registrationDate <= :end)")
